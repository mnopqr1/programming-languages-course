...
(* TODO: how to avoid nonexhaustive match? *)

(* Problem 1 *)

fun make_silly_json (i : int) =
    let fun make_silly_list (i : int, m : int, acc : json list) =
	    case i of
	        0 => acc
	      | _  => make_silly_list(i - 1, m, Object [("n", Num (int_to_real (m-i+1))), ("b", True)] :: acc)
    in
	Array (make_silly_list(i, i, []))
    end;
...
fun one_fields(j : json) =
    case j of
	Object ys => let fun field_names(xs : (string * json) list, acc : string list) =
			 case xs of
			     (k,v) :: xs' => field_names(xs', k :: acc)
			   | [] => acc
		     in
			 field_names(ys, [])
		     end
...
fun recursive_no_field_repeats (j : json) =
    let fun check_array (xs : json list) =
	    case xs of
		[] => true
	      | x :: xs' => recursive_no_field_repeats(x) andalso check_array(xs')
	fun check_inside (kvs : (string * json) list) =
	    case kvs of
		[] => true
	      | (k, v) :: kvs' => recursive_no_field_repeats(v) andalso check_inside(kvs')
    in	
	case j of
	    Num _ => true
	  | String _ => true
	  ...
	  | True => true
	  | Null => true
	  | Array xs => check_array xs
	  | Object kvs => no_repeats (one_fields (Object kvs) ) andalso check_inside (kvs)
    end;
